...
import { getTrades, getTradeDetails } from '@store/actions/tradesActions';

import { activeTradeIdSelector, tradesByUserSelector } from '@selectors/tradesSelectors';
...
export const TradesList = () => {
  ...
  const yourTrades: Array<ITrade> = 
    useSelector(tradesByUserSelector(currentUserId));
  ...
  const tradeCkickHandler = useCallback((tradeId: number) => {
  ...
  return (
    <>
      <span 
        ...
        tabIndex={0}
      >
        <img className='trades-menu__icon' src={iconPath} alt='trades menu icon' />
      </span>
