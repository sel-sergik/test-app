...
export const tradesByUserSelector = (currentUserId: number) => 
  createSelector(
    tradesSelector,
    (tradesList) => isSeller(currentUserId)
...
export const chatReadMessagesSelector = 
  (tradeId: number, currentUserId: number) => {
    return createSelector(
      (state: ITradeChatsState) => state.tradeChats,
      chats => chats.find((chat) => 
        chat.tradeId === tradeId)?.messages.filter(message => 
          message.isRead || message.userId === currentUserId)
    );
  };
...
export const chatUnreadMessagesSelector =   
  (tradeId: number, currentUserId: number) => {
    return createSelector(
      (state: ITradeChatsState) => state.tradeChats,
      chats => chats.find((chat) => chat.tradeId === tradeId)?.messages
...
export const interlocutorNameSelector = 
  (currentUserId: number, tradeId: number) => {
    return createSelector(
      relatedTradeSelector(tradeId),
      trade => isSeller(currentUserId) ? trade?.buyer.name : trade?.seller.name
    );
  };